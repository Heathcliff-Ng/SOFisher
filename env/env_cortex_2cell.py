...
Samping in Spatio Omics with discrete action
"""
...
class SpatOmics_dis():
    def __init__(self, args, exp, use_map= True):
        ...
        file_path = os.path.join('AD_sets_2cell','{}_tau_pos.npy'.format(exp))
        self.tau_pos = np.load(file_path)
        file_path = os.path.join('AD_sets_2cell','{}_Abeta_pos.npy'.format(exp))
        self.Abeta_pos = np.load(file_path)

        # other cells
        self.cells_pos = []
        for cell in ['L45_IT', 'L56_NP', 'L5_IT', 'L5_PT', 'L23_IT', 'L6_CT', 'L6_IT', 'Astrocytes', 
    ...
    def step(self, action, rand=False, isEval=False):
        ...
        r_Abeta = np.sum(Abeta_counts)
        r_tau = np.sum(tau_counts)
        r_AD = r_Abeta * r_tau

        if isEval:
            reward = 5 * r_AD
        else:
            reward = 1*r_overlap + 5*r_Abeta * r_tau + r_Abeta + r_tau
            if r_Abeta * r_tau > 2:
                 self.success += 1
            if self.success > 9:
                self.done = True
    ...
    def reset(self):
        ...
        cell_counts, Abeta_counts, tau_counts = self.measure()
        if np.sum(Abeta_counts)* np.sum(tau_counts) == 0:
        ...
        if self.use_map:
            ...
            state = np.concatenate((np.array([x, y]), mk))
    ...
    def warm_start_b(self, x, y):
        ...
        while x < self.x_max - self.rs/2:
            ...
            while y < self.y_max - self.rs/2:
                y = y + self.rs
                self.pos_sampling = [x, y]
                _, Abeta_counts, tau_counts = self.measure()
                if np.sum(Abeta_counts)* np.sum(tau_counts) > 0:
                    sampling_ended = True
                    break
            if sampling_ended:
